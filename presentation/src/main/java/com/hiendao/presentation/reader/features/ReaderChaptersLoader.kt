...
import com.hiendao.presentation.reader.domain.ChapterStats
import com.hiendao.presentation.reader.domain.ChapterUrl
...
internal class ReaderChaptersLoader(
    ...
    fun getItemContext(itemIndex: Int, chapterUrl: String): ReadingChapterPosStats? {
        ...
        return ReadingChapterPosStats(
            ...
            chapterUrl = chapterStats.chapter.id,
        )
    }
    ...
    fun isChapterIndexLoaded(chapterIndex: Int): Boolean {
        return orderedChapters.getOrNull(chapterIndex)?.id
    ...
    private suspend fun loadRestartedInitialChapter(
        ...
        val index = orderedChapters.indexOfFirst { it.id == chapterLastState.chapterUrl }
        if (index == -1) {
    ...
    private suspend fun loadInitialChapter(
        ...
        val initialPosition = readerRepository.getInitialChapterItemPosition(
            bookUrl = bookUrl,
            chapterIndex = chapter.lastReadPosition,
            chapter = chapter,
        )
    ...
    private suspend fun addChapter(
        ...
        when (val res = readerRepository.downloadChapter(chapter.id)) {
            is Response.Success -> {
                ...
                withContext(Dispatchers.Main.immediate) {
                    chaptersStats[chapter.id] = ChapterStats(
                        chapter = chapter,
