...
import com.hiendao.presentation.reader.domain.ChapterLoaded
import com.hiendao.presentation.reader.domain.ChapterState
import com.hiendao.presentation.reader.domain.ReaderItem
import com.hiendao.presentation.reader.domain.ReaderState
...
import com.hiendao.presentation.reader.features.ReaderChaptersLoader
import com.hiendao.presentation.reader.features.ReaderLiveTranslation
import com.hiendao.presentation.reader.features.ReaderTextToSpeech
import com.hiendao.presentation.reader.services.NarratorMediaControlsService
import com.hiendao.presentation.reader.tools.ChaptersIsReadRoutine
    ...
    private fun initLoadData() {
        scope.launch {
            ...
            val chaptersList = async(Dispatchers.Default) {
                orderedChapters.also { it.addAll(appRepository.bookChapters.chapters(bookUrl).map { it.toDomain() }) }
            }
            val chapterIndex = async(Dispatchers.Default) {
                chaptersList.await().indexOfFirst { it.id == chapterUrl }
            }
            chaptersList.await()
        ...
        scope.launch {
            readerTextToSpeech.reachedChapterEndFlowChapterIndex.collect { chapterIndex ->
                withContext(Dispatchers.Main.immediate) {
                    ...
                    if (readerChaptersLoader.loadedChapters.contains(chapterItem.id)) {
